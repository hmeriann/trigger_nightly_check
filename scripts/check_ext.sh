...
extensions=('arrow' 'autocomplete' 'avro' 'aws' 'azure' 'delta' 'excel' 'fts' 'httpfs' 'iceberg' 'icu' 'inet' 'jemalloc' 'json' 'motherduck' 'mysql_scanner' 'parquet' 'postgres_scanner' 'shell' 'spatial' 'sqlite_scanner' 'sqlsmith' 'substrait' 'tpcds' 'tpch' 'vss')
...
for platform in ${platforms[@]}; do
    for extension in ${extensions[@]}; do
        ...
        if (cat ${extension}-${sha}-${platform}.txt | grep -o $sha); then
